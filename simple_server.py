...
def fix_django_template(content):
    """Convert Django template syntax to Flask/static paths"""
    ...
    content = content.replace("{% if is_superuser %}", "<!-- ")
    content = content.replace("{% endif %}", " -->")
    content = content.replace("{{ tier_info.name }}", "Free")
    # Fix theme toggle button
    content = content.replace('onclick="toggleTheme()"', 'onclick="window.themeManager.toggle()"')
    return content
...
def index():
    ...
    with open('templates/landing_clean.html', 'r') as f:
        content = f.read()
    return fix_django_template(content)
...
if __name__ == '__main__':
    print("=" * 60)
    print("🛡️  BugBounty Arsenal - Simple Server")
    print("=" * 60)
