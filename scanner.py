    ...
    try:
        ...
        for det in ACTIVE_DETECTORS:
            ...
            for f in findings:
                ...
                # --- Screenshot integration only for medium/high/critical ---
                severity = (f_record.get("severity") or "").lower()
                if severity in ("medium", "high", "critical"):
                    screenshot_dir = "screenshots"
                    os.makedirs(screenshot_dir, exist_ok=True)
                    screenshot_path = os.path.join(
                        screenshot_dir,
                        f"screenshot_{hash(url)}_{findings.index(f)}.png"
                    )
                    try:
                        take_screenshot(url, screenshot_path)
                        f_record["screenshot_path"] = screenshot_path
                    except Exception as e:
                        logger.warning(f"Screenshot failed for {url}: {e}")
                # --- End screenshot integration ---
        ...
        for det in PASSIVE_DETECTORS:
            ...
            for f in findings:
                ...
                # --- Screenshot integration only for medium/high/critical ---
                severity = (f_record.get("severity") or "").lower()
                if severity in ("medium", "high", "critical") and f.get("evidence_details") is not None:
                    screenshot_dir = "screenshots"
                    os.makedirs(screenshot_dir, exist_ok=True)
                    screenshot_path = os.path.join(
                        screenshot_dir,
